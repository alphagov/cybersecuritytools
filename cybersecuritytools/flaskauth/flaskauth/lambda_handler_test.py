...
@pytest.fixture(scope="session")
def get_root(alb_https_odic_get_root: Dict[str, Any]) -> Dict[str, Any]:
...
def test_lambda_handler_returns_dict(get_root: Dict[str, Any]) -> None:
    """lambda handler should return a dict to AWS' calling function"""
    assert isinstance(get_root, dict)


def test_lambda_handler_dict_has_body(get_root: Dict[str, Any]) -> None:
    """The response has to have a 'body' key"""
    assert "body" in get_root


def test_lambda_handler_dict_has_statusCode(get_root: Dict[str, Any]) -> None:
    """The response has to have a 'statusCode' key"""
    assert "statusCode" in get_root


def test_lambda_handler_dict_has_headers(get_root: Dict[str, Any]) -> None:
    """The response has to have a 'headers' key"""
    assert "headers" in get_root


def test_lambda_handler_dict_has_content_type(get_root: Dict[str, Any]) -> None:
